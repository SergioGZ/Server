...
items["medkit"] = {"Kit medico","Para reanimar personas.",nil,0.5}
items["dirty_money"] = {"Dinero negro","Dinero ganado ilegalmente.",nil,0}
items["repairkit"] = {"Kit reparacion","Para reparar vehiculos.",nil,0.5}

-- money
items["money"] = {"Dinero","Dinero empaquetado.",function(args)
  local choices = {}
  local idname = args[1]
  ...
  choices["Unpack"] = {function(player,choice,mod)
    ...
    if user_id ~= nil then
      ...
      vRP.prompt(player, "¿Cuanto vas a desempaquetar? (max "..amount..")", "", function(player,ramount)
...
-- money binder
items["money_binder"] = {"Paquete","Empaqueta 1000€.",function(args)
  local choices = {}
  local idname = args[1]

  choices["Empaquetar dinero"] = {function(player,choice,mod) -- bind the money
    local user_id = vRP.getUserId(player)
    if user_id ~= nil then
...
local wammo_choices = function(args)
  ...
  choices["Cargar"] = {function(player,choice)
    local user_id = vRP.getUserId(player)
    if user_id ~= nil then
      local amount = vRP.getInventoryItemAmount(user_id, fullidname)
      vRP.prompt(player, "¿Cantidad a cargar? (max "..amount..")", "", function(player,ramount)
        ramount = parseInt(ramount)
