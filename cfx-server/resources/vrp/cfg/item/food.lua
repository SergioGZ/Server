    ...
    choices[act] = {function(player,choice)
      ...
      if user_id ~= nil then
        if vRP.tryGetInventoryItem(user_id,idname,1,false) then
          ...
          if ftype == "drink" then
            vRPclient.notify(player,{"~b~ Bebiendo "..name.."."})
            play_drink(player)
...
items["water"] = {"Botella de agua","", gen("drink",0,-25),0.5}
items["milk"] = {"Leche","", gen("drink",0,-5),0.5}
items["coffee"] = {"Cafe","", gen("drink",0,-10),0.2}
items["tea"] = {"Te","", gen("drink",0,-15),0.2}
items["icetea"] = {"Te helado","", gen("drink",0,-20), 0.5}
items["orangejuice"] = {"Zumo de naranja.","", gen("drink",0,-25),0.5}
...
-- create Breed item
items["bread"] = {"Pan","", gen("eat",-10,0),0.5}
